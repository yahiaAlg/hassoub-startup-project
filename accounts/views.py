...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if not username or not email or not password:
            messages.error(request, 'اسم المستخدم والبريد الإلكتروني وكلمة المرور مطلوبة!')
...
def profile(request):
    ...
    # Get user stats
    completed_lessons = UserLesson.objects.filter(user=request.user, is_completed=True).count()
    in_progress_lessons = UserLesson.objects.filter(user=request.user, is_completed=False).count()
    completed_scenarios = UserScenario.objects.filter(user=request.user, status='completed').count()
...
def user_dashboard(request):
    ...
    # Recent lessons
    recent_lessons = UserLesson.objects.filter(
        user=request.user
    ).select_related('lesson', 'lesson__path').order_by('-started_at')[:5]
    
    # Recent scenarios
    recent_scenarios = UserScenario.objects.filter(
        user=request.user
    ).select_related('scenario').order_by('-started_at')[:5]
    ...
    # Statistics
    total_lessons = UserLesson.objects.filter(user=request.user, is_completed=True).count()
    total_scenarios = UserScenario.objects.filter(user=request.user, status='completed').count()
...
def parent_dashboard(request):
    ...
    for child in children:
        ...
        # Get child stats
        completed_lessons = UserLesson.objects.filter(user=child, is_completed=True).count()
        completed_scenarios = UserScenario.objects.filter(user=child, status='completed').count()
        total_points = child_profile.total_points
        certificates = Certificate.objects.filter(user=child).count()
        
        # Recent activity
        recent_lessons = UserLesson.objects.filter(user=child).order_by('-started_at')[:3]
        recent_achievements = UserAchievement.objects.filter(user=child).order_by('-earned_at')[:3]
