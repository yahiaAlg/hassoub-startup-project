...
def scenario_list(request):
    ...
    for scenario in scenarios:
        ...
        best_score = UserScenario.objects.filter(
            user=request.user,
            scenario=scenario,
            status='completed'
        ).aggregate(best=Sum('final_profit'))['best'] or 0
...
def buy_product(request, user_scenario_id):
    ...
    user_scenario = get_object_or_404(UserScenario, id=user_scenario_id, user=request.user)
...
def scenario_result(request, user_scenario_id):
    ...
    total_purchases = transactions.filter(transaction_type='purchase').aggregate(
        total=Sum('amount')
    )['total'] or 0
